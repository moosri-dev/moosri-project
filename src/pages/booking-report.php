                    ...
                    <div class="row print-booking">
                        <div class="col-md-6">
                            <div class="order-header">
                                <span>ชื่อหมอนวด:</span><span class="order-value"> <?php echo $uname;?></span>
                                &nbsp&nbsp&nbsp&nbsp
                                <span>วันที่นวด:</span><span class="order-value"> <?php echo $_GET['ddate'];?></span>
                            </div>
                        </div>
                    ...
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">จำนวนเวลา</th>
                        ...
                        <tbody>
                            <?php
                                ...
                                ,d.bk_date as ddate, b.bk_name as bname, b.bk_time as btime, b.bk_cost as cost 
                                FROM hm_booking b INNER JOIN hm_booking_details d ON b.bk_id = d.bk_id_fk 
                                WHERE d.bk_date =? AND d.hm_user_id = ? AND d.status = ?';  
                             
                         
                                if($stmt = $mysqli->prepare($sql)){
                                    $status = 2;
                                    $stmt->bind_param("sii",$_GET['ddate'],$_GET['id'],$status);
                                    $stmt->execute();
                                    $result  = $stmt->get_result();
                                    $rows = 1;
                                    while($rs=$result->fetch_object()){
                                        ...
                                        echo '<td class="text-center">'.$rs->btime.' นาที</td>';
                        ...
                        <tfoot>
                            <?php
                                $sql= 'SELECT SUM(b.bk_time) as tTotal, SUM(b.bk_cost) as cTotal 
                                FROM hm_booking b INNER JOIN hm_booking_details d ON b.bk_id = d.bk_id_fk  
                                WHERE d.bk_date = ? AND d.hm_user_id = ? AND d.status = ?';  
